            ...
            if (entity != null) {
                ...
                doc.select("table#PostBox>tbody>tr>td").parallelStream().forEach(element -> {
                    if (element.select("span").size() > 0) {
                        ...
                        switch (title) {
                        case ("【所属黄页】"):
                            grabContent51Ca.setService_category(element.text());
                            break;
                        case ("【更新时间】"):
                            grabContent51Ca.setService_update_time(element.text());
                            break;
                        case ("【服务价格】"):
                            grabContent51Ca.setService_price(element.text());
                            break;
                        case ("【联系人】"):
                            grabContent51Ca.setService_contact(element.text());
                            break;
                        case ("【联系电话】"):
                            grabContent51Ca.setService_tel(null == grabContent51Ca.getService_tel() ? element.text() + ";" : grabContent51Ca.getService_tel() + element.text() + ";");
                            break;
                        case ("【电子邮件】"):
                            grabContent51Ca.setService_mail(element.select("a").attr("href").replaceFirst("mailto:", ""));
                            break;
                        case ("【其他电话】"):
                            grabContent51Ca.setService_tel(null == grabContent51Ca.getService_tel() ? element.text() + ";" : grabContent51Ca.getService_tel() + element.text() + ";");
                            break;
                        case ("【服务地区】"):
                            grabContent51Ca.setService_area(element.text());
                            break;
                        case ("【地图信息】"):
                            grabContent51Ca.setService_map(element.select("a").attr("href"));
                            break;
                        case ("【具体位置】"):
                            grabContent51Ca.setService_address(element.text());
                            break;
                        }
                    }
                });
            ...
            if (entity != null) {
                ...
                doc.select("table#PostBox>tbody>tr>td").parallelStream().forEach(element -> {
                    if (element.select("span").size() > 0) {
                        ...
                        switch (title) {
                        case ("【服务地区】"):
                            nbInfo.setAreaTags(element.text());
                            break;
                        case ("【地图信息】"):
                            nbInfo.setMapGoogle(element.select("a").attr("href"));
                            break;
                        }
                    }
                });
